...
talent-lab 是简历分析与个人求职管理场景的桌面优先工作台，视觉关键词为：专业、克制、清晰、紧凑、易扫描。

- 信息优先于装饰，主要层级依靠字号、字重、间距和边框建立。
- 界面只使用一个焦茶暖木主色（深胡桃棕）作为主要强调色，成功、警告、错误色仅表达语义状态。
- 浅色主题为纸张米色（`background` #F8F6F1），深色主题为炭木色（`background` #1F1A18）；纸张感由底色、边框、衬线标题和留白形成，不使用纹理图片或渐变光晕。
...
| `border` | 卡片、分隔线、非交互容器（装饰性细线，不保证 3:1） |
| `control-border` | 输入框、选择框、未选中 Checkbox、outline 按钮等交互控件的边界，需达到非文字 3:1 |
...
专用令牌仅在对应场景使用：`terminal-*` 用于 SSE 解析终端（两种主题下都保持深色，强调色为暖铜色，红黄绿状态灯保持原样），`chart-*` 用于图表，`scrim` 用于遮罩，`primary-deep` 用于登录品牌面板。

状态不能只依赖颜色表达，必须同时提供文字或图标。半透明语义背景采用 `primary/10`、`success/10` 等形式，边框通常采用同色 `/20`。

## 4. 字体与文字层级

全局正文、表格、按钮、表单和数据字体为 Inter Variable，中文回退依次使用系统无衬线字体、苹方、冬青黑体和微软雅黑。

展示标题使用 `font-display` 衬线字族（Iowan Old Style、Songti SC、STSong、Noto Serif CJK SC、Source Han Serif SC、SimSun、Georgia、Times New Roman、serif，均为系统字体，不打包字体文件）。`font-display` 只允许出现在：

- 登录页主品牌标题
- 桌面和移动侧栏的 Talent Lab 字标
- 页面内容区的一级标题（搭配 `tracking-tight`）
- 候选人姓名、职位标题等文档型主标题
- Dashboard 的主要区块标题
- 简历画像的主标题

表格、按钮、标签、Badge、输入框、筛选器、分页器、图表标签、卡片字段名和数字评分必须保持无衬线。
...
- 应用不设置全局横向 Header，页面一级标题是当前位置的唯一主标题。
- 桌面端从 `lg` 起使用距视口 16px、圆角 28px 的全高应用窗口。
- 桌面侧栏展开宽度为 224px，收起宽度为 72px；折叠按钮固定在侧栏与内容区分隔线上。
- 移动端使用左上角 44px 悬浮汉堡按钮；展开后显示宽 280px、圆角 20px 的悬浮导航菜单，不使用全高侧滑抽屉。
- 常规页面最大宽度为 1280px，即 `max-w-7xl`。
- 页面内边距为移动端 16px、桌面端 40-48px；移动端顶部为悬浮按钮预留 80px。
- 应用壳使用 `h-dvh` 并让主内容区独立滚动，路由切换后主内容回到顶部。
...
- 桌面应用窗口使用 `rounded-shell`，对应 28px。
- 卡片、面板、表格外框统一使用 `rounded-lg`，对应 16px。
- 按钮、输入框、选择框统一使用 `rounded-md`，对应 10px。
- 微型按钮和内部标记可使用 `rounded-sm`，对应 6px；Badge 和标签允许使用胶囊圆角。
- 常规容器使用 `border border-border`，分组列表使用单向分隔线，避免重复套边框。
- 阴影只用于应用窗口、悬浮导航、抽屉、错误通知或确有层级差的浮层。普通卡片依靠边框和背景区分。
...
- 使用 `border-control`、`bg-background`、`text-sm` 和 2px `ring` 焦点反馈。
...
- 未选中时使用背景色与 `control-border` 边框；选中时使用焦茶暖木主色和对勾。
